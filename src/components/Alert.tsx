...
const Alert = ({alerts, mobile=false, locale}: {alerts:any, mobile?:boolean, locale: string | undefined}) => {
  ...
  useEffect(()=> {
    ...
    setCurrentAlerts(alerts.alerts.filter((e: any)=> {
  ...
  return (
    <div ref={box} className={`${mobile ? "mr-4 lg:hidden " : ""}`}>
      <div className="">
        {alerts.alerts.type === "obs" && <Image className="hover:brightness-90 active:brightness-75 cursor-pointer duration-100" onClick={()=> setAlertShown(!alertShown)} src={"/obs.svg"} alt={"obs"} width={mobile ? 26 : 32} height={mobile ? 26 : 32}/>}
        {alerts.alerts.type === "notis" && <Image className="hover:brightness-90 mr-3 active:brightness-75 cursor-pointer duration-100" onClick={()=> setAlertShown(!alertShown)} src={"/notis.svg"} alt={"obs"} width={mobile ? 26 : 32} height={mobile ? 26 : 32}/>}
        <div onClick={()=>setAlertShown(false)} className={`fixed lg:min-w-100 lg:max-w-180 lg:max-h-80 mr-[4vw] lg:mr-2 w-[92vw] max-h-[50vh] overflow-scroll z-40 bg-bg-white/60 backdrop-blur-xl p-5 cursor-pointer
          ...
          <div className="w-fit py-1 pr-10 text-body md:text-body-desktop text-text-black font-albert font-bold">{locale=== "/sv" ? alerts.alerts.title_sv : alerts.alerts.title_en}</div>
          <div className="w-fit py-2 text-body md:text-body-desktop text-text-black font-albert font-normal">{locale=== "/sv" ? alerts.alerts.text_sv : alerts.alerts.text_en}</div>
        </div>
      </div>
    </div>
  )
