...
const WhiteGridBox = ({logoUrl, messageTitle, messageTextIndex, backgroundWhite}:{logoUrl:string, messageTitle:string, messageTextIndex:string, backgroundWhite?:boolean}) => {
  ...
  return (
    <div className="my-3 lg-w-[30%] w-full px-4 lg:px-[1.5%]">
      <div onClick={()=>{setIseOpen(!isOpen)}} className={`cursor-pointer pt-3 h-14 max-w-220 flex items-center justify-between bg-primary ${backgroundWhite ? "lg:bg-bg-white lg:text-primary lg:border-x lg:border-t lg:border-gray-300" : "md:bg-primary"} 
      ...
      <div className={`${isOpen ? "flex lg:flex": "hidden lg:flex"} px-4 pb-4 max-w-220 pt-4 lg:pt-0 lg:pl-17 rounded-b-xl text-body lg:text-body-desktop lg:min-h-38 font-albert 
