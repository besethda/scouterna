...
const ContentSection = ({ sectionLayout, page, background = "white", padding = "both", display = "both", child = false }: { sectionLayout: string[], page: string, background?: string, padding?: string, display?: string, child?: boolean }) => {
  ...
  const createLayout = (characterLayout: string[]) => {
    let result = characterLayout.reduce((acc, element: string, index) => {
      ...
      if (element === "l") {
        acc.push(<ul key={index} className="w-fit text-body my-3 md:text-body-desktop text-text-black font-albert lg:max-w-[63%] list-disc pl-5"><li>{(messages as any)[page]?.[`${element}${count.length}`]}</li></ul>)
      } else {
        acc.push(<div key={index} className={`
          ${element === "p" ? "w-fit py-2 text-body md:text-body-desktop text-text-white font-albert lg:max-w-[63%]" :
            element === "w" ? "w-fit py-2 text-body-bold md:text-body-bold-desktop text-text-black font-fraunces" :
              element === "h" ? "w-fit py-2 text-h1 md:text-h1-desktop text-primary font-fraunces font-bold" :
                element === "t" ? "w-fit py-2 text-h2 md:text-h2-desktop text-text-white font-fraunces font-bold" :
  ...
  return (
    <div className={`w-full flex justify-center ${background === "white" ? "bg-bg-white" : background === "blue" ? "bg-primary" : background === "mobile" ? "bg-bg-blue md:bg-bg-white" : background === "desktop" ? "bg-bg-white md:bg-bg-blue" : background === "large" ? "bg-bg-white lg:bg-bg-blue" : "bg-bg-white"}
      `}>
