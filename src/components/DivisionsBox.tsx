  ...
  return (
    <div className={`${colorReference[division][0]} flex flex-col flex-wrap border border-b-4 m-1 rounded-2xl px-6 py-3 ${boxStyle !== "image" ? boxStyle === "info" ? "md:w-[49.5%] mx-[.25%]" : "md:w-[24%] mx-[0.5%]" : "md:max-h-81 lg:max-h-70 md:max-w-250"}`}>
      ...
      <div className={`${boxStyle === "info" && "md:hidden"} flex grow justify-center md:justify-start pt-2 text-primary items-end md:pb-3 order-4 md:order-3`}>
        {boxStyle !== "image" ? <Link href={messages?.path + `/groups/${division}`} className={`${colorReference[division][0]} ${colorReference[division][1]} h-fit cursor-pointer hover:brightness-140 active:brightness-80 duration-150 text-center font-albert text-body md:text-body-desktop border rounded-3xl py-3.5 w-full`}>{`${messages?.division.basic_message} ${messages?.division[`${division}_title`] ?? ""}`}</Link>
                              : <Link href={messages?.path + `/groups/${division}`} className={`${colorReference[division][0]} ${colorReference[division][1]} h-fit cursor-pointer hover:brightness-140 active:brightness-80 duration-150 font-albert flex items-center justify-center text-body md:text-body-desktop border rounded-3xl`}>
